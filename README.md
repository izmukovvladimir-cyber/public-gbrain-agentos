...
## Главное правило: gbrain — единственный канал координации роя

> **Каждый агент ОБЯЗАН вести всю коммуникацию со своим роем через свой gbrain** — не через локальные файлы, не через разовые каналы, не через догадки о том, что знают остальные. Это не опциональная обвязка, а условие связности роя. Пропустил gbrain — другие не видят твою работу, ты не видишь их: решения дублируются, контекст теряется, **агенты забывают**.

Что это значит на практике для каждого агента, каждую сессию:

- **Recall до действия.** Перед нетривиальной задачей — `recall(query)` по общему мозгу: коллега мог это уже решить, зафиксировать или словить ту же ошибку. Локальная память — fallback, не источник.
- **Notify, а не предполагай.** Передать работу другому агенту или отдать нужный им результат — через `swarm.notify(to_agent, payload)` / `broadcast` / `escalate`. Свой inbox забирать `list_my_pending` на старте сессии, `ack` обработанное.
- **Decisions / runbooks / error-patterns — в память**, а не только в локальный markdown, чтобы следующий агент (или следующий ты) их достал, а не переоткрывал.
- **Работа — через tasks** (`task_*`), чтобы её состояние было видно всему рою, а не заперто в одной сессии.

Это правило вшито в [`agent-template/templates/CLAUDE.md.template`](agent-template/templates/CLAUDE.md.template) — каждый сгенерированный агент получает его в always-loaded контекст, поэтому забыть его нельзя.
